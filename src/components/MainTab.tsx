...
function getLaneHeadToHead(p1: string, p2: string, lane: LineKey, matches: Match[]) {
  ...
  for (const m of matches) {
    const a = (m.team_a?.[lane] || '').trim();
    const b = (m.team_b?.[lane] || '').trim();
    const wt = (m as any).winning_team as 'Red'|'Blue'|undefined;
    if (!wt) continue;
...
// 중복 없이 맞라인 전적 기반 10명 뽑기 - 벨런스 + 중복 방지 핵심
function findBalancedLineup(allStreamers: string[], matches: Match[]) {
  ...
  function dfs(idx: number, used: Set<string>, cur: {lane:LineKey; p1:string; p2:string; games:number}[], total:number) {
    ...
    for (const c of list.slice(0, 8)) {
      if (used.has(c.p1) || used.has(c.p2)) continue; // 중복 방지!
      ...
      if (best && best.total > 30) break; // 충분히 좋으면 조기 종료
    }
  }
  ...
  if (!best) {
    // 맞라인 전적 자체가 부족하면 주포지션 기반으로 중복 없이 10명 뽑기 (fallback)
    ...
    return { red, blue, totalGames: 0, pairs: [] };
  }
...
// 시너지
function getPairRate(p1:string,p2:string,pairMap?:any,stats?:any){ if(!p1||!p2) return 50; const k1=`${p1}|${p2}`,k2=`${p2}|${p1}`; if(pairMap instanceof Map){ const d=pairMap.get(k1)||pairMap.get(k2); if(d){ if(typeof d==='number') return d; if(d.winRate) return d.winRate*100; if(d.rate) return d.rate; if(d.wins!=null) return d.total?d.wins/d.total*100:50; } } if(stats?.pairWinrates){ const d=stats.pairWinrates[k1]||stats.pairWinrates[k2]; if(d){ if(typeof d==='number') return d; if(d.winRate) return d.winRate*100; if(d.rate) return d.rate; } } if(pairMap && typeof pairMap==='object'){ const d=(pairMap as any)[k1]||(pairMap as any)[k2]; if(d){ if(typeof d==='number') return d; if(d.winRate) return d.winRate*100; if(d.rate) return d.rate; } } return 50; }
function calcSynergy(team:TeamRoster,pairMap?:any,stats?:any){ const pls=Object.values(team).filter(Boolean); if(pls.length<2) return 50; let sum=0,cnt=0; for(let i=0;i<pls.length;i++) for(let j=i+1;j<pls.length;j++){ sum+=getPairRate(pls[i],pls[j],pairMap,stats); cnt++; } return cnt?sum/cnt:50; }
function calcWinRates(red:TeamRoster,blue:TeamRoster,pairMap?:any,stats?:any){ const sR=calcSynergy(red,pairMap,stats); const sB=calcSynergy(blue,pairMap,stats); const tot=sR+sB; if(!tot) return {red:50,blue:50,sR,sB}; return {red:sR/tot*100, blue:sB/tot*100, sR, sB}; }
function findOptimal(playersByPos:Record<LineKey,[string,string]>,pairMap?:any,stats?:any){ const posList=LINE_KEYS as LineKey[]; let best:any=null, bestScore=Infinity; for(let mask=0; mask < (1<<posList.length); mask++){ const red:TeamRoster={top:'',jgl:'',mid:'',adc:'',sup:''}; const blue:TeamRoster={top:'',jgl:'',mid:'',adc:'',sup:''}; for(let i=0;i<posList.length;i++){ const p=posList[i]; const [a,b]=playersByPos[p]; if((mask & (1<<i))===0){ red[p]=a; blue[p]=b; } else { red[p]=b; blue[p]=a; } } const {red:wrR,blue:wrB,sR,sB}=calcWinRates(red,blue,pairMap,stats); const score=Math.abs(wrR-wrB)*0.7+Math.abs(sR-sB)*0.3-(sR+sB)*0.01; if(score<bestScore){ bestScore=score; best={red,blue,wrR,wrB}; } } return best; }
...
export const MainTab: React.FC<MainTabProps> = ({ matches, allStreamers, stats, pairMap, onToast }) => {
  ...
  const handleFill = useCallback(()=>{
    ...
    if (dup.length>0) {
      console.error('중복 발견:', dup);
      onToast(`중복 오류 감지: ${dup.join(', ')} - 다시 시도해주세요`);
      return;
    }
    ...
    for (const {lane, p1, p2} of (result as any).pairs || []) {
    ...
    onToast(`맞라인 전적 기반 벨런스 맞춤 완료! 총 ${result.totalGames}판의 맞라인 전적 (중복 없이 10명)`);
  ...
  const handleAnalyze = useCallback(()=>{ if(!isFull){ onToast('10명이 모두 채워져야 분석 가능합니다.'); return; } const r=calcWinRates(redTeam,blueTeam,pairMap,stats); setWinRate(r); onToast(`시너지 분석: Red ${r.red.toFixed(1)}% vs Blue ${r.blue.toFixed(1)}%`); }, [isFull, redTeam, blueTeam, pairMap, stats, onToast]);
  const handleOptimal = useCallback(()=>{ if(!isFull){ onToast('10명이 모두 채워져야 최적 재배치가 가능합니다.'); return; } const byPos:Record<LineKey,[string,string]>={top:[redTeam.top,blueTeam.top],jgl:[redTeam.jgl,blueTeam.jgl],mid:[redTeam.mid,blueTeam.mid],adc:[redTeam.adc,blueTeam.adc],sup:[redTeam.sup,blueTeam.sup]}; const best=findOptimal(byPos,pairMap,stats); if(best){ setRedTeam(best.red); setBlueTeam(best.blue); setWinRate({red:best.wrR,blue:best.wrB,sR:0,sB:0}); onToast(`최적 재배치 완료! Red ${best.wrR.toFixed(1)}% vs Blue ${best.wrB.toFixed(1)}%`); } }, [isFull, redTeam, blueTeam, pairMap, stats, onToast]);

  return (
    <div className="max-w-[1100px] mx-auto space-y-4">
      <div className="bg-[#1a1a2e] border border-[#2a2a4a] rounded-2xl px-5 py-3.5 flex items-center justify-between gap-4">
        <div className="flex items-center gap-2.5 text-[13px] font-bold text-white"><span className="text-[16px]">⚡</span>맞라인 전적 기반 5:5 벨런스 추천 - 같은 선수 중복 없음</div>
      </div>
      <div className="bg-[#12121a] border border-[#1e1e2a] rounded-[20px] p-5">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-[14px] font-black text-white">라인별 팀 입력 <span className="text-[#5a5a70] font-normal text-[11px] ml-1">(맞라인 전적 + 중복 방지)</span></h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-[#1a1010]/50 border border-[#3a1e1e] rounded-2xl p-4">
            ...
            {(LINE_KEYS as LineKey[]).map(pos=><div key={`red-${pos}`} className="flex items-center gap-3 mb-2.5"><div className="w-[36px] text-[11px] font-bold text-[#8a8aa0] uppercase">{pos}</div><div className="flex-1 relative"><input list="main-players" value={redTeam[pos]} onChange={e=>{ setRedTeam(p=>({...p,[pos]:e.target.value})); setWinRate(null); }} placeholder="스트리머 이름" className="w-full h-[36px] bg-[#08080c] border border-[#2a1e1e] rounded-full px-4 text-[12px] text-white placeholder:text-[#5a5a70] focus:outline-none focus:border-[#ef4444]/50" /><span className="absolute right-2 top-1/2 -translate-y-1/2 text-[9px] px-1.5 py-0.5 rounded-full bg-[#2a1e1e] text-[#8a8aa0]">주:{playerMainPos.get(redTeam[pos])?.toUpperCase()||'-'}</span></div></div>)}
          </div>
          <div className="bg-[#101a2a]/50 border border-[#1e2a4a] rounded-2xl p-4">
            ...
            {(LINE_KEYS as LineKey[]).map(pos=><div key={`blue-${pos}`} className="flex items-center gap-3 mb-2.5"><div className="w-[36px] text-[11px] font-bold text-[#8a8aa0] uppercase">{pos}</div><div className="flex-1 relative"><input list="main-players" value={blueTeam[pos]} onChange={e=>{ setBlueTeam(p=>({...p,[pos]:e.target.value})); setWinRate(null); }} placeholder="스트리머 이름" className="w-full h-[36px] bg-[#08080c] border border-[#1e2a4a] rounded-full px-4 text-[12px] text-white placeholder:text-[#5a5a70] focus:outline-none focus:border-[#3b82f6]/50" /><span className="absolute right-2 top-1/2 -translate-y-1/2 text-[9px] px-1.5 py-0.5 rounded-full bg-[#1e2a4a] text-[#8a8aa0]">주:{playerMainPos.get(blueTeam[pos])?.toUpperCase()||'-'}</span></div></div>)}
          </div>
        </div>
        ...
        {!isFull && <div className="mt-4 text-[11px] text-[#f59e0b] bg-[#f59e0b]/10 border border-[#f59e0b]/20 rounded-full px-3 py-2 text-center">10명이 모두 채워져야 분석 가능합니다. (현재 {Object.values(redTeam).filter(Boolean).length + Object.values(blueTeam).filter(Boolean).length}/10명)</div>}

        {winRate && <div className="mt-4 bg-[#08080c] border border-[#1e1e2a] rounded-xl p-3 flex items-center justify-between text-[12px]"><div className="text-[#8a8aa0]">예상 승률: Red <b className="text-[#f87171]">{winRate.red.toFixed(1)}%</b> vs Blue <b className="text-[#60a5fa]">{winRate.blue.toFixed(1)}%</b></div><div className="text-[10px] text-[#5a5a70]">맞라인 전적 기반 • 중복 없음</div></div>}
