...
export const CHAMPION_KR_TO_EN: Record<string, string> = {
  ...
  "일라오이": "Illaoi",
  "자르반 4세": "JarvanIV", "자르반": "JarvanIV", "자야": "Xayah", "자이라": "Zyra", "자크": "Zac", "잔나": "Janna",
...
export const SOOP_POPULAR_STREAMERS = [
  "우리밍_", "꿀탱죽었다", "육면", "미키.", "로코블", "논킹.", "사코타임0", "상이너", "이하름", "꽁령이."
];
...
export function getChampionEnName(krName: string): string | null {
  const normalized = normalizeChampionName(krName);
  if (!normalized) return null;
  if (CHAMPION_KR_TO_EN[normalized]) return CHAMPION_KR_TO_EN[normalized];
  const withoutDot = normalized.replace(/\.$/, "");
  if (CHAMPION_KR_TO_EN[withoutDot]) return CHAMPION_KR_TO_EN[withoutDot];
  if (/^[a-zA-Z0-9]+$/.test(normalized)) return normalized;
  const noSpace = normalized.replace(/\s/g, "");
  if (CHAMPION_KR_TO_EN[noSpace]) return CHAMPION_KR_TO_EN[noSpace];
  return null;
}
...
export function parseKdaString(kda: string): { kills: number; deaths: number; assists: number } | null {
  if (!kda) return null;
  try {
    const parts = kda.trim().split(/\s*\/\s*|\s+/);
    if (parts.length >= 3) {
      return {
